...
title: "A Text Corpus based on the Hansard of the Australian Federal Parliament between 1901 and 2018"
thanks: "Thank you to Monica Alexander for her helpful suggestions; and to the UC Berkeley Demography Department for the use of their computing resources. The code and data for this paper is available at: https://github.com/RohanAlexander/hansard. Comments and suggestions on the `r format(Sys.time(), '%d %B %Y')` version of this paper welcome at: rohan.alexander@anu.edu.au."
author:
...
As such, we use the Correlated Topic Model (CTM) of @BleiLafferty2007 to obtain topic distributions. The CTM is a modification of LDA that allows for correlations between topics. More detail about the CTM is in Appendix \@ref(correlatedtopicmodelsection). There are various methods to help determine the appropriate number of topics to specify, and we further discuss the implications given our setting later in the paper. However the main implication is that the optimal number of topics was too large to be tractable by our analysis model, and too large to be easily visualised or examined. For this reason we manually group the topics identified by the CTM into those of the Comparative Agendas Project. It is these proportions that we use as an input to our analysis model.
...
Define $\theta_{cdp}$ to be the proportion of topic $p$ on day $d$ in chamber $c$ (where chamber refers to either the House of Representatives or the Senate). Note that the $\theta_{c,d,1:P}$ for $p = 1,2,\dots, P = 19$ are equal to the estimated values of $\theta_{cd}$ from the CTM after we group them into the Comparative Agendas Project topics. We assume that the majority of variation in topics by day is across sitting periods, $s$, where a sitting period is defined as any group of days that are less than one week apart. Using this definition, there are 822 sitting periods over the period 1901 to 2018 inclusive. Appendix \@ref(hansardsummarystatistics) contains more information about the sitting patterns over the course of the year, which have changed considerably since Federation.
...
Firstly, we describe the results of the CTM approach, which defined 80 topics over the period 1901 to 2018, and which we then further reduced to match the Comparative Agendas Project. We then describe the results of the Bayesian analysis model, which identified prime ministers, elections and other events that were associated with a change in the topics discussed. 
...
Of the 36 prime ministers over this period, we find that 17 of them were significantly different to the prime minister that preceeded them, after we account for and remove some that had especially short terms. The first three that are significant occur in the first decade after Federation. To a certain extent, this may be due to the variety of the issues that had to be addressed in those initial years. The fourth prime minister that was significantly different to his predecessor was Joseph Cook. This coincided with the initial period of World War I and it may be that this is the reason for difference, compared with his predecessor, and successor Andrew Fisher.

Stanley Bruce is significantly different from his predecessor Billy Hughes, even though they were ostensibly from the same party. Hughes had been prime minister for over six years, and Bruce was a compromise choice in order to form a coalition.
