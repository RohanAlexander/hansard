...
The main output of the STM that is of interest to us is the proportion of each topic in each document. We would like to know how those proportions change given events. But with around 80 **[UPDATE]** topics for each of the roughly 54,331 **[UPDATE]** sitting days across the parliaments and houses the data are still too noisy to easily visualise changes around events. Instead, we use a Bayesian hierarchical Dirichlet model that takes those proportions as an input. 

We index each proportion, $\gamma$, by sitting period $s$, time $t$ and topic $p$, that is $\gamma_{stp}$. For this purpose time, $t$, refers to the day index within a sitting period, and a sitting period is any collection of days separated from another collection of days by at least a week. Our model is:
