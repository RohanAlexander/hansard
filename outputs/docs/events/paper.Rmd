...
**[Not sure this is being used - might be better in the other paper.]**
...
## Analysis model
We are interested in the effect of political, economic, and other events on Australian parliamentary discussion. Political events are those related to a change of government or an election. Economic events are defined by substantial changes in various economic measures, such as the onset of the Great Depression or floating the currency. Other events are those such as the 9/11 attacks. The full list of events that we consider are detailed in Appendix \@ref(eventdetails).
...
One output of topic modelling is a probability distribution over the topics for each document. In our case the documents are each day's text. We analyse events by focusing on those proportions, $\gamma$, which we index by sitting period $s$, time $t$ and topic $p$, that is $\gamma_{stp}$. Note that time $t$ refers to the day index within a sitting period which is any collection of days separated from other days by at least a week. Our analysis model is:

\begin{align}
\gamma_{stp} &\sim \mbox{Dirichlet}(\mu_{stp}) (\#eq:distributionforgammas) \\ 
\log \mu_{stp} &= \alpha_{sp} + \delta_{stp}. (\#eq:meanforgammas) \\
\alpha_{s1} &= 0 \\
\alpha_{sp} &\sim \mbox{Normal}\left(\eta_{g[s]}, \sigma^2_g\right) \mbox{ for } p>1 \\
\delta_{s,t,p} &\sim \mbox{Normal}\left(\delta_{s,t-1,p}, \sigma^2_{\delta}\right) \\
\sigma^2_{\delta} &\sim \mbox{Uniform}(0, 40) 
\end{align} 

Equation \@ref(eq:distributionforgammas) describes the proportion of each day within a sitting period given to a particular topic as a draw from a Dirichlet distribution parameterised by $\mu_{stp}$. The Dirichlet distribution is the multivariate extension of the Beta distribution, and the parameter controls where the 'weight' of the distribution lies. Equation \@ref(eq:meanforgammas) describes that distribution parameter, $\mu_{stp}$, as the sum of $\alpha_{sp}$ which is **X**, and $\delta_{stp}$, which is **Y**. These are themselves parameters. The primary aim of the modelling task is to find an appropriate and meaningful functional form for $\mu$. We consider $\alpha_{sp}$ as a draw from a Normal distribution with a mean that depends on **A** and a variance that depends on **B**. Similarly, $\delta_{stp}$ is modelled as a draw from a Normal distribution with a mean that depends on the preceding $\delta$ and some variance which is drawn from the Uniform distribution.

**[Does the variance parameter used in Equation 4 need to be specified too?]**

An illustration of the validity of our analysis model using simulated data is in Appendix \@ref(modeldetailsandsimulation). 
...
# Results
We are interested in considering the effect of various events on what is talked about in Australia's parliaments. Each of the parliaments and their houses are treated independently here. Future work could expand the model to better understand, and allow, for correlation between them. 
