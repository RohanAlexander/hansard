...
thanks: "Thank you to John Tang, Zach Ward, Tim Hatton, Martine Mariotti, Tianyi Wang, Matt Jacob, Leslie Root, Jill Sheppard, Matthew Kerby, Chris Cochrane and Dan Simpson for their helpful suggestions; and to the UC Berkeley Demography Department for the use of their computing resources. Comments and suggestions on the `r format(Sys.time(), '%d %B %Y')` version of this paper welcome at: rohan.alexander@anu.edu.au."
author:
...
The CTM output of interest is the proportion of each topic appearing in each document, that is, the $\theta_d$. The aim of this stage of the modeling process is to analyse how the distribution of topics changes in relation to different types of events. But with 40 topics for each of the roughly 14,551 chamber-sitting-days the data are still too noisy to easily visualise changes around events.
...
The topic proportions on day $d$ are modeled in reference to their membership of a particular sitting period $s$. Firstly, we assume that each distribution of topics, $\theta_{d,1:P}$ for each day is a draw from a Dirichlet distribution with mean parameter $\mu_{s[d],1:P}$:
...
The goal of the model is to relate these proportions to government $g$ at time $d$, and also the days since the most recent election, $e$, while account for underlying time trends. The mean parameters $\mu_{sp}$ are modeled on the log scale as
...
- $\sum_{k=1}^{K} \beta_{p,k} \cdot x_{s,k}$ is the underlying time trend, modeled using splines: $x_{s,k}$ is the $k$th basis spline in sitting period $s$ and $\beta_{p,k}$ is a cofficient on the $k$th basis spline; and
...
We summarise our results in terms of elections Table \@ref(tab:electionsresults) and in terms of governments in Table \@ref(tab:governmentsresults). These tables focus on elections and governments that were different to the ones that preceeded them. 
...
We summarise the periods where parliamentary discussion was significantly different in Table X. **[Oh boy. Not sure what to do here.]**

Finally, we summarise our results over time by looking at the standard deviation of sitting period estaimtes, around the mean election effect (Figure \@ref(fig:sigmaelection))
...
Of the 36 different governments, we find that 12 of them are significantly different to the government that preceeded them. However, two of these -- Page and the first Menezies government -- are likely due to the short length of the Page Government, and should be ignored, leaving only ten. The earliest of these is the second Menzies government starting in 1949, and these ten are concentrated in the second half of our sample, with three of them being in the past twenty years.

The significant difference of the second Menzies Government is associated with a variety of changes. The Chifley Government that preceeded it had finsih  For instance there was a large increase in the number of lower house seats at the 1949 election. This brought many new politicians into the house and it may be that they brought different topics.


Of the 45 general elections that have been held we find that 12 of them define periods that were significantly different to their immediate predecessor. 1974, 1980, 1990, 1998, 2004, and 2007 stand out as elections where the government did not change, but the model suggests there was considerable change in the topics discussed in parliament.
...
**[Add some nice for specific changes for the discussion section?]**
...
Add discussions about weakness of the model. The main weakness of this approach is that it does not appropriately propagate uncertainty and future models should improve on this. 
