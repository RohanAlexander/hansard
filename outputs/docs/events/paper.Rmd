...
The goal of our modeling strategy is twofold. Firstly, we want to topic modeling __(REF)__ to summarise all text contained in Hansard into meaningful topics, which reduce the dimensionality of the text data and capture the main themes discussed in parliament over time. Secondly, we want to relate the resulting topic distributions to temporal trends, changes and events, such as governement changes, elections, and other external events. 

We use a Correlated Topic Model (CTM) __(REF)__ to obtain estimated topic distributions over time. We consider these topic distributions as reduced dimension inputs that can be analysed within another model. We then formulate a Bayesian hierarchical Dirichlet model to assess changes in the topic distributions in relation to events of interest. __In Appendix \@ref(word2vec), we include an alternative approach that follows @Taddy2015 by using word2vec, which more closely uses words, rather than topics, as an input.__
...
As mentioned in the previous section, one of the limitations of LDA is that the model assumes that the presence of one topic is not correlated with the presence of another topic. Indeed, the corrleation between topics is not modeled or accounted for. However, in relaity, we would expect Hansard topics related the army may be more commonly found with topics related to the navy, but less commonly with topics related to banking. The goal of the CTM __(REF)__ is to account for this correlation between topics, in order to produce more realistic and stable topic distributions over time. 
