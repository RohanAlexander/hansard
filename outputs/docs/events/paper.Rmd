...
In the following section, we briefly describe the topic modelling approach, which is to use the CTM, which is a natural extension of the Latent Dirichlet Allocation (LDA) model [@Blei2003latent], before discussing the Bayesian hierarchical Dirichlet analysis model used to investigate changes in topics. More detail on the topic modelling is available for readers that may be less familiar with it in Appendix \@ref(hansardworkflow) 
