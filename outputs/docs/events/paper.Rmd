...
thanks: "Thank you to John Tang, Zach Ward, Tim Hatton, Martine Mariotti, Tianyi Wang, Matt Jacob, Leslie Root, Jill Sheppard, Matthew Kerby, Chris Cochrane and Dan Simpson for their helpful suggestions; and to the UC Berkeley Demography Department for the use of their computing resources. Our dataset is available via an R package detailed at https://hansardr.com. Comments and suggestions on the `r format(Sys.time(), '%d %B %Y')` version of this paper welcome at: rohan.alexander@anu.edu.au."
author:
...
This is, the main difference between the CTM and LDA is that it replaces the assumption of the Dirichlet distribution with a more flexible logistic Normal distribution. That distribution can incorporate a covariance structure across the topics. The remainder of the steps of the document generating process are identical to LDA. 
...
Figure \@ref(fig:exampletopics) illustrates a CTM output only a five per cent sample from the Australian House of Representatives between 1901 and 2017. It shows how each day's parliamentary discussion can be apportioned to a topic and highlights how these proportions change over time.
...
We run the model in JAGS using the \texttt{rjags} package of @Plummer2018. __An illustration of the validity of our analysis model using simulated data is in Appendix \@ref(modeldetailsandsimulation). __
...
Add discussions about weakenss of the model. The main weakness of this approach is that it does not appropriately propagate uncertainty and future models should improve on this. 

Add discussion of the weakness of the dataset.
