...
institute: Presentation at the Max Planck Institute for Demographic Research, Rostock, Germany.
...
- Create a dataset of what was said in the Australian Federal Parliament from 1901 through to 2018 based on available public records. 
...
- Changing the prime minister tends be associated with topic changes even when the party in power does not change.
- Elections that do not result in a change in prime minister are rarely associated with topic changes.
- The effect of these types of events is much more pronounced in the second half of our sample, and especially in the past two decades.
...
- There are 14,680 days of publicly available Hansard records across both chambers of the Australian Federal Parliament.
...
knitr::include_graphics("../../../figures/topics_examples_three_one.png")
```
...
- This relates the proportions of each topic to underlying time trends, changes in prime minister and elections. 
...
- Define $\theta_{dp}$ to be the proportion of topic $p$ on day $d$. Note that the $\theta_{d,1:P}$ for $p = 1,2,\dots P=80$ are equal to the estimated values of $\theta_d$ from the CTM. 
- We assume that the majority of variation in topics is across sitting periods $s$, where a sitting period is defined as any group of days that are less than one week apart. Using this definition, there are 745 sitting periods over the period 1901 to 2018 inclusive.
...
The goal of the model is to relate these proportions to prime minister $g$ at time $d$, and also the days since the most recent election, $e$, while accounting for underlying time trends. 
...
- $\alpha_{g[s],p}$ is the mean effect for prime minister $g$ (which covers sitting period $s$) and topic $p$;
...
## Analysis model - Prime minister effects

The prime minister term $\alpha_{g[s],p}$ assumes there is some underlying mean effect of each prime minister on the topic distribution. We place uninformative priors on each of these parameters: $\alpha_{g[s],p} \sim \mbox{Normal}(0, 100)$
...
The sitting period-specific random effect $\delta_{s,p}$ allows the topic distributions in some sitting periods to be different than expected based on prime minister and election effects:
$$
...
## Analysis model - Significantly different prime ministers
...
## Analysis model - Significantly different prime ministers (cont.)
...
## Analysis model - All prime ministers
