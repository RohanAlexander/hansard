---
title: "Dirichlet with election groups"
author: "Monica Alexander"
date: "10/3/2018"
output: html_document
---

Now we have a model going, let's try and add in some groupings based on changes in elections. Hopefully then we will be able to visualize the 'average' topic distributions across governments. Let's try
...
```{r}
dr <- d %>% filter(electionCounter>43)
```
...
mu_res %>% 
  ...
  facet_wrap(~topic)
ggsave("./elections/mus_election.pdf", width = 15, height = 10)
```
...
write_csv(alpha_res, "./elections/alpha_res.csv")
```
...
ggsave("./elections/alphas_election.pdf", width = 30, height = 20)
```
