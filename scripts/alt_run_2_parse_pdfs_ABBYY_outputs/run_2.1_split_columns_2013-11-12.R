...
use_this_path_to_get_csvs  <- "/Volumes/Hansard/parsed/federal/senate/run_1_output"
...
# The two columns is only an issue up to 12 November 2013 so remove the file if it is before 2013-11-12
file_names_tibble <- tibble(file_name = file_names, file_date = file_names) %>% 
  mutate(file_date = str_replace(file_date, ".csv", ""),
         ...
         file_date = str_replace(file_date, "/Volumes/Hansard/parsed/federal/senate/run_1_output", ""),
...
use_this_path_to_save_csvs  <- "/Volumes/Hansard/parsed/federal/senate/run_2_output"
