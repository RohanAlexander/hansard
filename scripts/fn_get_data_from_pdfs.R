...
file_names <-
  list.files(
    path = "/Volumes/SanDisk/hansard_pdfs", # Change this as required
    pattern = "*.pdf",
    recursive = TRUE,
    full.names = TRUE
  ) 
...
get_text_from_PDFs <- function(name_of_input_PDF_file, name_of_output_txt_file) {
# Iterate over files from here
document <- Corpus(URISource(name_of_input_PDF_file), readerControl = list(reader = read))
