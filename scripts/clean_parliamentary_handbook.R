...
parliamentary_handbook <-
  read_csv("data/parliamentary_handbook.csv", col_names = TRUE)


#### Change the dates ####
# Change classes to dates
parliamentary_handbook$dateOfElection <-
  dmy(parliamentary_handbook$dateOfElection)
parliamentary_handbook$dateCeasedToBeMember <-
  dmy(parliamentary_handbook$dateCeasedToBeMember)
...
check_changes <-
  gender_based_on_name %>% group_by(name) %>% summarise(n_distinct(gender))
...
# Make the changes
parliamentary_handbook$gender[parliamentary_handbook$hansardName %in% c(
  "BARTLETT, Kerry",
  "BLAND, Francis",
  "BRIGGS, Jamie",
  "CHAPMAN, Hedley",
...
# females <- parliamentary_handbook$hansardName[parliamentary_handbook$gender == "female"]
# write.csv(females, "all_females.csv")
