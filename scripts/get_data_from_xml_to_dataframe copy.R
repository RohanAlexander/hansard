...
file_names <- file_names[c(1:2)] # Further reduce the number of files to one while getting it working
...
for (i in 1:length(file_names)) { # Loop into each file
  this_hansard_as_xml <- read_xml(file_names[1]) # Read file i as XML, change i to 1 while testing
  ...
  this_hansard_as_tibble <- 
    ...
    map_df(function(x) {
      ...
      # neighbours_check <-
      #   xml_find_first(x, "./following::para")
      
      neighbours_check <-
        xml_find_first(x, "../following-sibling::para intersect ../following-sibling::*[name()!='para'][1]/preceding-sibling::para
")


      # xml_find_all(test_hansard, "//following-sibling::para")
...
test_hansard <- read_xml(
  ...
  <para>- Has the attention of the Prime Minister</para>
  </talk.start>
  <para>Australia should aim at a stabilized economy with an employed rate of 8 per cent. This plan, he said, was preferable to full employment. Complete stability was neither attainablenor desirable, said the Professor, who is Economic Adviser to the Bank of New South Wales. </para>
  <para>TAKE AIM CLOSELY </para>
...
<interjection>
            <talk.start>
              ...
              <para>- Why were the eighteen children sent back to Alice Springs? </para>
            </talk.start>
          </interjection>
  </speech>
